...
def evaluate(model, x, y_true, output_dict=False):
    y_pred = model.predict(x)
    rpt = classification_report(y_true, y_pred, output_dict=False)
...
def main():
    ...
    save_dir = os.path.join(cfg.work_dirs, 'models')
    mkdirs(save_dir)

    train_dataset = pd.read_csv(cfg.train_feature)
    # 先求normalization参数
    normal_stats = None
    if cfg.normalization == 'global':
        train_feature = pd.read_csv(cfg.train_feature)
        test_feature = pd.read_csv(cfg.test_feature)
    ...
    for train_model in tqdm(cfg.train_models):
        ...
        model = train(normed_train_data, train_labels, train_model['name'], save_name,
                      train_model['random_state'], train_model['params'])
        print('\n\n{} {} {}'.format('=' * 36, train_model['name'], '=' * 36))
        # rpt_row = [train_model['name']]

        val_dataset = pd.read_csv(cfg.val_feature)
        ...
        rpt_str = evaluate(model, normed_val_data, val_labels)
        print(rpt_str)
